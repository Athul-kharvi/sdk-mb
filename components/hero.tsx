        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
          {images.map((_, i) => (
            <button
              key={i}
              onClick={() => setIdx(i)}
              className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${i === idx ? 'bg-[#D4A017] w-4' : 'bg-white/50'}`}
            />
          ))}
        </div>
      )}
...
export function Hero({ desktopImages, mobileImages }: HeroProps) {
  const dImgs = desktopImages?.length ? desktopImages : ['/images/hero_image.png']
  const mImgs = mobileImages?.length ? mobileImages : (desktopImages?.length ? desktopImages : ['/images/hero_image_mobile.png'])

  return (
    <>
      {/* ── MOBILE ── */}
      <section className="sm:hidden relative w-full overflow-hidden" style={{ height: '92svh' }}>
        <Slideshow images={mImgs} />
